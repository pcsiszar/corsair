...
A defense happens after the attacker has rolled their dice, but before the outcome is determined. The process is as
follows:

1. **The Defender Narrates:** The defending player describes how their character is trying to resist the action. Are
   they dodging, raising a shield, shouting a counter-argument, or bracing for impact?
2. **The GM Chooses an Attribute:** Based on the defender's narration, the GM chooses a single, relevant Attribute for
   the defense.
3. **The Defense Roll:** The defender rolls a number of d10s equal to the chosen Attribute's value.
4. **Mitigation:** For every die in the defense roll that scores an 8 or higher, the defender cancels out one of the
...
Now, the GM decides the drone gets to defend itself.
...
**1. The Defense Roll:** The drone makes a defense roll using its **Evasion** Attribute. It rolls **3d10**.
...
This can be used during both the Action Roll and the Success Roll of a Complex Test. When defending against a Teamwork
action, any Mitigation points generated by the defender are used to remove the highest dice from across all the
participating players' pools.

The base requirement for being able to work together to accomplish something is that all players must aim for the same
end result, even if the approach for the players might be different. This means aligning the end result both
mechanically and narrative wise. A group effort to persuade cannot be done when one of the players tries to intimidate
...
* **Defending against a Test:** 1 Action Token
...
* **Example:** Jax is in a firefight in a cantina and wants to shoot his pistol (a Complex Test, 2 Tokens) at a drone
  ...
  quality of the cover based on the Success Test, which might provide an Upgrade to his defense rolls. Because the
